...
- [ ] 029 — pretooluse-deny-is-inert-under-bypass-permissions — full track [hardened] — the five PreToolUse guards deny correctly when invoked directly and the identical live Edit passes, so every rule that calls them a hard, unskippable block is wrong in the mode these sessions actually run. Found as rocky H13.
- [ ] 030 — unlisted-fires-forever-on-an-optional-callee — spec-only — a CORE file that calls a project script behind `[ -f ]` is a use, not a dependency; on rocky that reports three files permanently and holds the tick guard red. A detector whose output is never empty is not a detector — its own words. Corroborated by bc84617, same class in agentcrm; but that fix (delete the reference) is unavailable here because these three are CALLED, not merely named. Found as rocky H13.
