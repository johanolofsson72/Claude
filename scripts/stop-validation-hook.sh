...
if [ -f "$MARKER" ]; then
  # Cross-platform mtime: try GNU stat (-c %Y) first, fall back to BSD/macOS
  # stat (-f %m). On GNU coreutils -f means --file-system, so the BSD form must
  # NOT run first or it prints filesystem info instead of an epoch.
