...
- `.claude/rules/scenarios.md` — living, diagram-led scenario map rooted at `specs/SCENARIOS.md`, in one of TWO layouts: **single-file** (everything in that one file — correct for almost every project) or **split** (the file keeps the use-case diagram and a per-feature index; each feature's flowchart and rows live in `specs/scenarios/<slug>.md`), which a project earns by outgrowing one file. `scripts/scenario-map-layout.sh` is the one predicate that decides which, and every consumer sources it. (Mermaid use-case diagram + per-feature user-flow flowchart + SC-id ledger; journey map / wireflow / storyboard on-demand). Each scenario carries a `☐→◐→✓` status; `✓` = validated working at runtime (four states: success / specific error / empty / loading). A gap or drift starts a scenario interview rather than letting a missed case slip into code (paths: `**/specs/**`, `**/spec*.md`, `**/SCENARIOS.md`)
...
- `.claude/rules/spec-interview.md` — per-spec anti-drift interview: EVERY spec carries 15–25 questions before source code is touched, recorded in `<spec-dir>/interview.md`; enforced by the `spec-interview-guard` PreToolUse hard block. Default is AUTO — Claude auto-answers the base with the recommended option (escalating genuinely-ambiguous ones) and asks the developer the overflow questions when a spec is flagged large/advanced; `SPEC_INTERVIEW_MODE=manual` forces fully-human answering. Sits right after `/speckit-specify`, before `/speckit-clarify`
- `.claude/rules/carve-budget.md` — the termination condition the ruleset never had. Every other rule creates rows (`validation-followup.md` surfaces findings, `spec-hardening.md` runs a checkpoint that exists to find things, `lane-handoff.md` turns uncovered work into a row); none said when to stop. Measured 2026-09-03 across five projects, the carve ratio — rows added per row ticked — was 2.15 (rocky), 2.08 (agentcrm), 1.30 (consultpilot): registers growing faster than they close, so the backlog never drains however fast you work. The rule caps carves at **2 per spec** and **depth 2**, sends harness/tooling defects to the template repo rather than a product register, and makes a ratio ≥1.3 over 10+ ticked rows a **convergence stop**. `scripts/register-convergence.sh` measures it; it runs in `project-maintenance.sh` and at SessionStart.
...
- `scripts/scenario-map-reminder-hook.sh` — PostToolUse advisory (never blocks). Fires when a spec/tasks file gains interactive behaviour but the scenario map has no rows for it — searching the index AND every `specs/scenarios/*.md`, because under the split layout most slugs live only in a feature file and an index-only search would report a gap for every well-mapped feature, which costs the advisory the credibility it entirely runs on; instructs Claude to START a scenario interview (capture happy/edge/adversarial/error/offline cases) rather than just jotting a note. Silent on template/scratch repos (no language marker). Pairs with `.claude/rules/scenarios.md`.
...
- `scripts/spec-register-orientation-hook.sh` — SessionStart advisory. Emits the spec-register status (totals + next row) and the `/clear`-before-big-spec banner, PLUS a **context-cost canary**: `wc -c` on `specs/INDEX.md`, `specs/SCENARIOS.md` and (under the split layout) each `specs/scenarios/*.md` measured SEPARATELY and never summed — a sum would fire permanently on a healthy split map — and past ~25 KB it warns that these per-spec-read files have ballooned and to run `scripts/archive-spec-history.sh` + read them targeted. Wired into SessionStart by `sync-core-hooks.py` (script-presence gated). Pairs with `.claude/rules/spec-register.md`. (Previously absent from this manifest — a project only received edits to it by accident; now copied like the other core hooks.)
- `scripts/scenario-map-layout.sh` — Sourceable helper, not a hook. `scenario_map_layout <root>` echoes `split` or `single_file`; `scenario_map_files <root>` lists every file that can hold SC rows. The ONE implementation of the layout predicate — consumers source it rather than re-deciding, because two hooks disagreeing about where the rows are is worse than either being wrong, the disagreement being silent. An empty `specs/scenarios/` counts as single-file.
- `scripts/scenario-map-rows.sh` — Extracts the SC ledger from a map (either layout) as normalized, sorted, diffable lines. Matches `~~SC-nnn~~` retired rows as well as live ones; the obvious `^\| SC-` pattern misses exactly the three rows whose ids must never be reused. `--summary` prints counts and the status histogram.
- `scripts/test-scenario-map-split.sh` — Proves a map restructure was lossless: extracts every row now and diffs against a committed pre-split baseline. The baseline is an artefact, never regenerated from the current map — that would make the check assert the map equals itself.
- `scripts/test-scenario-map-index.py` — Nine invariants asserting the index has not drifted from the feature files it summarizes: the split's own new failure mode, since two documents can disagree where one could not. Compares the per-feature status tally PER STATUS, not by total — a `◐` flipped to `✓` and rewritten as one larger `✓` count keeps the sum and loses the truth. Silent on the single-file layout, where there is no index to drift from.
- `scripts/test-scenario-map-fixtures.sh` / `test-scenario-map-layouts.sh` / `test-scenario-map-reminder.sh` / `test-scenario-map-canary.sh` — Fixture builder plus three harnesses covering both layouts. Every harness asserts the single-file cases are unchanged, because that is the behaviour 41 of 42 projects actually run.
...
- `scripts/repeat-failure-guard-hook.sh` — PostToolUse hook on `Bash`. Makes CLAUDE.md's "Max 3 attempts per problem" enforceable instead of aspirational. Fingerprints VERIFICATION commands only (`dotnet build|test|stryker`, `npm test|run build|lint|typecheck`, `pytest`, `go test`, `cargo test`, `flutter test|build|analyze`, `maestro`/`patrol`, `mvn`/`gradlew`, `tsc`, `eslint`, `vitest`, `jest`, `playwright test`) and counts CONSECUTIVE failures per fingerprint in `.claude/state/attempts/` (gitignored, TTL-pruned, default 6h). At 3 it injects "change strategy, not syntax"; at 5 it escalates to "this is a hard blocker — surface it". Classification is **three-state** (H6s): a recognized failure counts up, a recognized success resets, and a run it cannot classify — or a payload whose `tool_response` it cannot read — leaves the failure counter **untouched** and counts on a separate `<fp>.unknown` streak that fires its own weaker nudge ("I cannot tell whether this passed") at the same limit. Never blocks, fails open, needs `jq`. Both signature sets are anchored and specific, and the positive set spans every stack the command matcher tracks — because a FALSE success resets a real spiral. Note the payload carries **no exit code**, so `Exit code: N` matches only when the command prints it itself; classification is text-heuristic, which is exactly why the third state exists. Tunables: `ATTEMPT_LIMIT` (3), `ATTEMPT_ESCALATE` (5), `ATTEMPT_TTL` (21600), `REPEAT_FAILURE_DISABLE=1`.
...
- `scripts/stack-marker-canary-hook.sh` — SessionStart advisory (never blocks). Re-derives the project's actual stack from its manifests (`package.json` dependency KEYS — `expo`/`react-native` vs `vite`/`next`/`@playwright/test`/`vue`/`svelte`/`astro`/`nuxt`/`@angular/core` — plus `pubspec.yaml` with a Flutter SDK, `eas.json`/`app.config.*`, and any `*.csproj`/`*.sln`, searched 3 levels deep excluding vendored trees) and compares it against `.claude/.sync-stack`. Speaks ONLY on a mismatch or a missing marker; silent otherwise, which is nearly every session. **Why it exists:** `.sync-stack` is written once and never re-derived, yet `template-autosync.sh` reads it as a GATE — a stale `testing=mobile` on a project that pivoted to web means the browser docs are withheld forever, the wrong docs get hand-patched, and the sync then skips them as "locally modified" too. Both failure modes are silent. puck ran a month that way: its destructive-test checklist prescribed Maestro and Patrol for an app that ships in a browser. The warning text deliberately tells you NOT to blind-flip the marker (flipping makes the next sync overwrite those docs) and walks the read → set → copy → verify-0-skips sequence. A `hybrid` marker on a one-sided repo is treated as a deliberate choice, not drift. Disable with `STACK_CANARY_DISABLE=1`.
...
if { [ -f specs/INDEX.md ] && grep -qE '^- \[[ x/!]\]' specs/INDEX.md; } \
  ...
  elif [ -d specs/scenarios ] && [ -n "$(ls -A specs/scenarios 2>/dev/null)" ]; then
    # Split layout: the index is SMALL by construction and its rows live in the feature
    # files. Do not read a thin SCENARIOS.md here as a thin map.
    echo "[OK] scenario map present (split layout: index + $(ls specs/scenarios/*.md 2>/dev/null | wc -l | tr -d ' ') feature files)"
  else
    echo "[OK] specs/SCENARIOS.md present (single-file layout)"
  fi
else
