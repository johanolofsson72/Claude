...
- **(BLOCKING)** Non-trivial feature/refactor/fix → run the full pipeline as **one task** (`specify → interview → clarify → elicit → plan → tasks → analyze → implement → tests → tla`); no permission stops between phases. `interview` and `clarify` run on all tracks; `elicit` on full/light only. Trivial-fix bypass needs an explicit one-sentence classification. See `.claude/rules/feature-pipeline.md`.
- **(BLOCKING)** Spec interview — **every** spec, right after `specify` and before `clarify`, answers **15–25 questions** via `AskUserQuestion` (one per turn, **auto-pick OFF**), recorded in `<spec-dir>/interview.md`. The `spec-interview-guard` hook hard-blocks source edits until ≥15 are answered. This is the anti-drift gate: `clarify` auto-picks silently, so the interview is where a human actually pins down scope, edge cases, error/empty/loading states, authorization, and non-goals. Never auto-generate the answers. See `.claude/rules/spec-interview.md`.
...
- **Spec interview (15–25 human-answered questions per spec, anti-drift hard gate)** → `.claude/rules/spec-interview.md`
