...
- [ ] 044 — traceability-gate-reports-zero-on-a-binary-file — spec-only — `validate-scenario-traceability.sh` scans the reference roots with grep; one binary file under a root makes the whole scan yield nothing, and it reports `coverage: 0 of N` with every row listed as uncovered rather than erroring. Found in fundit: TLC wrote scratch to `tests/tla/states/` and the gate went from 172/179 to 0/182 with no signal that it had stopped working. Fix: `grep -I` / `--binary-files=without-match`, and refuse rather than report zero when a scan finds no ids at all.
