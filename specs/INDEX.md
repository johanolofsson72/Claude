...
- [ ] 030 — unlisted-fires-forever-on-an-optional-callee — spec-only — a CORE file that calls a project script behind `[ -f ]` is a use, not a dependency; on rocky that reports three files permanently and holds the tick guard red. A detector whose output is never empty is not a detector — its own words. Found as rocky H13.
