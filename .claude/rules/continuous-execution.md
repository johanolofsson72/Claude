...
- Relaying spec-kit 1.0's `/speckit-implement` prompt — *"Some checklists have unchecked items. Do you want to proceed with implementation anyway? (yes/no)"* — to the developer. It is a permission-check on work already authorized, shipped inside a third-party skill. Judge the unchecked items instead: a real gap is a **finding** surfaced per `.claude/rules/validation-followup.md` (a legitimate stop, and a far more useful one); anything else is bookkeeping, so tick it or note it and continue. See the override in `.claude/rules/feature-pipeline.md`.
