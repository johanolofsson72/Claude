...
- `.claude/rules/spec-interview.md` — per-spec anti-drift interview: EVERY spec answers 15–25 human-answered questions (auto-pick OFF) before source code is touched, recorded in `<spec-dir>/interview.md`; enforced by the `spec-interview-guard` PreToolUse hard block. Sits right after `/speckit-specify`, before `/speckit-clarify`
