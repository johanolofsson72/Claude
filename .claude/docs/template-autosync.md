...
**The manifest is the copy loop's record, and only that.** The sync also writes through four other routes — the `settings.json` seed and its hook-merge sites, the two wiring helpers' own `scripts/` copies, `.specify/extensions/.registry`, and `.claude/.sync-stack` — and none of them may enter the manifest, because every reader of it assumes copy-loop semantics. Those paths get a separate `# wrote <sha256> <path>` record in the same file: the bytes the sync *left* at a path whose content it did not author end to end. It is read for exactly one question — *is my own write still uncommitted* — and never to justify an overwrite. Without it, `[stranded]` answered for one route in five and missed `settings.json`, the file every hook lives in.
